...
object Score
{
  ...
  def apply (s: String) : Score =
  {
    ...
    scoreType match
    {
      case "mate" => Score(ScoreType.Mate, scoreValue.toInt)
      case "cp" => Score(ScoreType.CentiPawns, scoreValue.toInt)
...
object Info
{
  val InfoRegex = """info depth (\d+) seldepth \d+ score (\w+ \w+) nodes (\d+) nps \d+ time (\d+) multipv (\d+) pv (.*)""".r
...
object BestMove
{
  val BestMoveRegex = """bestmove (\w+) ponder (\w+)""".r
